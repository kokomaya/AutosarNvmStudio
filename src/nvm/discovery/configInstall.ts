...
async function ensureLayoutRoot(fsPath: string): Promise<void> {
...
export function registerConfigInstallCommand(): vscode.Disposable {
	return vscode.commands.registerCommand("nvmStudio.nvm.installConfigsFromUrl", async () => {
		...
		try {
			...
			if (entries.length === 0) {
				void vscode.window.showInformationMessage("Server has no layout configs to install.");
				return;
			}
			...
			if (installed === 0) {
				void vscode.window.showWarningMessage("No layout configs were downloaded.");
				return;
			}
